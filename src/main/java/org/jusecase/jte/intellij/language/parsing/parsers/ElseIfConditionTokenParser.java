...
public class ElseIfConditionTokenParser extends AbstractTokenParser {
    ...
    public boolean hasToken(int position) {
        if (lexer.getCurrentState() == JteLexer.CONTENT_STATE_JAVA_ELSEIF_BEGIN) {
            if (hasToken(position, "(", JteTokenTypes.CONDITION_BEGIN)) {
                lexer.setCurrentState(JteLexer.CONTENT_STATE_JAVA_ELSEIF_CONDITION);
                return true;
            }
        } else if (lexer.getCurrentState() == JteLexer.CONTENT_STATE_JAVA_ELSEIF_CONDITION && lexer.getCurrentCount() <= 0) {
            if (hasToken(position, ")", JteTokenTypes.CONDITION_END)) {
                lexer.setCurrentCount(0);
                lexer.setCurrentState(JteLexer.CONTENT_STATE_JAVA_ELSEIF_END);
                return true;
            }
        }
