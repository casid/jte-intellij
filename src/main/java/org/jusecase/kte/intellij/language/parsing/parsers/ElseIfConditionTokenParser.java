...
public class ElseIfConditionTokenParser extends AbstractTokenParser {
    ...
    public ElseIfConditionTokenParser(KteLexer lexer) {
        this.lexer = lexer;
    }

    @Override
    public boolean hasToken(int position) {
        if (lexer.getCurrentState() == KteLexer.CONTENT_STATE_ELSEIF_BEGIN) {
            if (hasToken(position, "(", KteTokenTypes.CONDITION_BEGIN)) {
                lexer.setCurrentState(KteLexer.CONTENT_STATE_ELSEIF_CONDITION);
                return true;
            }
        } else if (lexer.getCurrentState() == KteLexer.CONTENT_STATE_ELSEIF_CONDITION && lexer.getCurrentCount() <= 0) {
            if (hasToken(position, ")", KteTokenTypes.CONDITION_END)) {
                lexer.setCurrentCount(0);
                lexer.setCurrentState(KteLexer.CONTENT_STATE_ELSEIF_END);
                return true;
            }
        }
