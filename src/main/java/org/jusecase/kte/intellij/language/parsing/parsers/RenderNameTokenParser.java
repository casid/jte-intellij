...
public class RenderNameTokenParser extends AbstractTokenParser {
    ...
    public RenderNameTokenParser(KteLexer lexer) {
        this.lexer = lexer;
    }

    @Override
    public boolean hasToken(int position) {
        if (lexer.getCurrentState() == KteLexer.CONTENT_STATE_RENDER_BEGIN) {
            if (hasToken(position, "(", KteTokenTypes.PARAMS_BEGIN)) {
                lexer.setCurrentState(KteLexer.CONTENT_STATE_RENDER_NAME);
                return true;
            }
        } else if (lexer.getCurrentState() == KteLexer.CONTENT_STATE_RENDER_NAME && lexer.getCurrentCount() <= 0) {
            if (hasToken(position, ")", KteTokenTypes.PARAMS_END)) {
                lexer.setCurrentCount(0);
                lexer.setCurrentState(KteLexer.CONTENT_STATE_RENDER_END);
                return true;
            }
        }
